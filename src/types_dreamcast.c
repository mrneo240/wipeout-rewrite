...
void mat4_set_yaw_pitch_roll(mat4_t *mat, vec3_t rot) {
  ...
  platform_sincos( rot.x, &sx, &cx);
  platform_sincos(-rot.y, &sy, &cy);
  platform_sincos(-rot.z, &sz, &cz);
...
void mat4_translate(mat4_t *mat, vec3_t translation) {
	...
	mat_translate(translation.x, translation.y, translation.z);
...
void mat4_mul(mat4_t *res, mat4_t *a, mat4_t *b) {
	mat_load((matrix_t *)a);
	mat_apply((matrix_t *)b);
	mat_store((matrix_t *)res);
