...
void ship_ai_update_intro_await_go(ship_t *self) {
	self->position.y = self->temp_target.y + platform_sin(self->update_timer * (80.0 + self->pilot * 3.0) * 30.0 * M_PI * 2.0 / 4096.0) * 32;
...
void ship_ai_update_race(ship_t *self) {
	...
	if (flags_not(self->flags, SHIP_FLYING)) {
		...
		float xy_dist = platform_sqrt(track_target.x * track_target.x + track_target.z * track_target.z);
