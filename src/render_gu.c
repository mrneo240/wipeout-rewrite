...
void render_init(vec2i_t size) {
  ...
  void *texman_aligned =
      (void *)((((unsigned int)texman_buffer + TEX_ALIGNMENT - 1) / TEX_ALIGNMENT) * TEX_ALIGNMENT);
...
void create_white_texture(void) {
  ...
  rgba_t white_pixels[4] = {rgba(128, 128, 128, 255), rgba(128, 128, 128, 255), rgba(128, 128, 128, 255),
...
static void render_setup_2d_projection_mat(void) {
  ...
  float nf = 1 / (near - far);
  projection_mat_2d = mat4(-2 * lr, 0, 0, 0, 0, -2 * bt, 0, 0, 0, 0, 2 * nf, 0, (left + right) * lr,
                           (top + bottom) * bt, (far + near) * nf, 1);
}
...
static void render_setup_3d_projection_mat() {
  ...
  projection_mat_3d = mat4(f / aspect, 0, 0, 0, 0, f, 0, 0, 0, 0, (FAR_PLANE + NEAR_PLANE) * nf, -1, 0, 0,
                           2 * FAR_PLANE * NEAR_PLANE * nf, 0);
}
...
void render_set_depth_offset(float offset) {
  ...
  sceGuDepthOffset((unsigned int)offset); /* I think we need a little more on psp because of
                                                                       16bit depth buffer */
}
...
void render_buffer_tris(tris_t tris, uint16_t texture_index, tris_texid_t *buffer_out) {
  //@Todo: do we truly need this?
  // error_if(texture_index >= textures_len, "Invalid texture %d", texture_index);
  ...
  (*buffer_out).tri = tris;
  (*buffer_out).texture_id = texture_index;
}
...
void render_push_2d_tile(vec2i_t pos, vec2i_t uv_offset, vec2i_t uv_size, vec2i_t size, rgba_t color,
